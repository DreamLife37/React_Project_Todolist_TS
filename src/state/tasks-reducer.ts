...
export const addTaskTC = (p: { todolistId: string, title: string }) => (dispatch: Dispatch) => {
    todolistsAPI.createTask(p.todolistId, p.title)
        .then((res) => {
...
export const updateTaskTitleTC = (todolistId: string, id: string, newTitle: string) => (dispatch: Dispatch, getState: () => AppRootStateType) => {
    const state = getState()
